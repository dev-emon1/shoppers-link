...
const API_VALUES = "http://127.0.0.1:8000/api/attributeValues";
const API_ATTRIBUTES = "http://127.0.0.1:8000/api/attributes";
...
const AttributeValue = () => {
  ...
  const fetchAttributes = useCallback(async () => {
    try {
      const res = await axios.get(API_ATTRIBUTES);
  ...
  const fetchValues = useCallback(async () => {
    try {
      const res = await axios.get(API_VALUES);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingValue) {
        await axios.put(`${API_VALUES}/${editingValue.id}`, payload);
        alert("✅ Attribute Value updated successfully!");
      } else {
        await axios.post(API_VALUES, payload);
