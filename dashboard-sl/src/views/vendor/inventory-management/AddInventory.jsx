          ...
          prev.map((p) => ({
            ...
            variants: p.variants?.map((v) =>
              Number(v.id) === Number(editingVariantId)
                ? {
                    ...v,
                    stock: res.data.new_stock || parseInt(editStockValue),
                  }
                : v,
            ),
          })),
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead className="bg-gradient-to-r from-main to-mainHover text-white">
                <tr>
                  <th className="px-8 py-2 text-left font-bold text-sm">
                    Product Name
                  </th>
                  <th className="px-8 py-2 text-left font-bold text-sm">
                    Variants (Stock)
              ...
              <tbody className="divide-y divide-gray-100">
                {filteredProducts.length === 0 ? (
                  <tr>
                    <td
                      colSpan="2"
                      className="text-center py-20 text-gray-500 text-md"
                    >
