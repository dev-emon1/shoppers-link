...
const CustomerDetails = () => {
  ...
  if (!customer)
    return (
      <div className="p-12 text-center text-red-600">Customer not found</div>
    );
  ...
  const deliveredOrders = orders.filter(
    (o) => getStatus(o) === "delivered" || getStatus(o) === "completed",
  );
  const pendingOrders = orders.filter((o) => getStatus(o) === "pending");
  const cancelledOrders = orders.filter((o) => getStatus(o) === "cancelled");
  ...
  const segment =
    totalOrders >= 20 || totalSpent >= 400000
      ? "VIP"
      : totalOrders >= 10 || totalSpent >= 200000
        ? "Loyal"
        : totalOrders <= 2 && totalOrders > 0
          ? "New"
          : totalOrders === 0
            ? "Inactive"
            : "Regular";
  ...
  return (
    <div className="px-6 bg-gray-50 min-h-screen">
      {/* Back Button */}
      <Link
        to="/vendor/users/customers"
        className="inline-flex items-center gap-2 text-main hover:underline mb-6 text-sm font-medium"
      >
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-2">
        ...
        {/* Cancelled */}
        <div className="bg-white rounded-xl shadow p-4 text-center">
          <p className="text-xl font-bold text-red-600">{totalCancelled}</p>
