...
const AllCustomersTablePage = () => {
  ...
  const fetchCustomers = useCallback(async () => {
    try {
      ...
      setMeta({
        current_page: Number(
          Array.isArray(payload.meta?.current_page)
            ? payload.meta.current_page[0]
            : payload.meta?.current_page || 1,
        ),
        last_page: Number(
          Array.isArray(payload.meta?.last_page)
            ? payload.meta.last_page[0]
            : payload.meta?.last_page || 1,
        ),
        per_page: Number(
          Array.isArray(payload.meta?.per_page)
            ? payload.meta.per_page[0]
            : payload.meta?.per_page || 10,
        ),
        total: Number(
          Array.isArray(payload.meta?.total)
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (item) => {
        ...
        return (
          <span className="font-bold text-xs">৳ {total.toLocaleString()}</span>
        );
      },
    },
    {
      ...
      <PageHeader
        ...
        rightActions={
          <div className="flex gap-2">
            <button
              onClick={exportToPDF}
              className="flex items-center gap-1 px-2 py-1 border border-main text-main rounded text-xs font-medium hover:bg-main hover:text-white transition-colors"
            >
              <TbPdf size={16} /> PDF
            </button>
            <button
