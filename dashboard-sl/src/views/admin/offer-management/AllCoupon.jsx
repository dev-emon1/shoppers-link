      ...
      <div className="flex flex-col sm:flex-row justify-between items-center mb-4 gap-4 bg-white p-4 rounded-2xl shadow-sm border border-gray-100">
        <div>
          ...
          <p className="text-gray-500 text-sm">
            Monitor performance and manage discount logic.
          </p>
        </div>
        <Link
          to="/admin/coupons/add-coupon"
          className="flex items-center gap-2 bg-main hover:bg-mainHover text-white px-4 py-2 rounded-xl transition-all shadow-lg shadow-blue-100 text-sm"
        >
          <Plus size={20} /> Add New Coupon
        </Link>
      </div>
      ...
      {coupons.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {coupons.map((coupon) => (
            <div
              key={coupon.id}
              className="bg-white border border-gray-100 rounded-3xl p-6 shadow-sm hover:shadow-xl transition-all relative group overflow-hidden flex flex-col justify-between"
            >
              <div>
                {/* Status & Type Badge */}
                <div className="flex justify-between items-start mb-4">
                  <div className="flex flex-col gap-1">
                    <span
                      className={`w-fit px-3 py-1 rounded-full text-[10px] font-bold tracking-widest uppercase ${coupon.is_active ? "bg-green-100 text-green-600" : "bg-red-100 text-red-600"}`}
                    >
                      {coupon.is_active ? "Active" : "Disabled"}
                    </span>
                    <span
                      className={`text-[10px] font-bold px-2 py-0.5 rounded-md w-fit ${coupon.vendor_id ? "text-orange-500 bg-orange-50" : "text-blue-500 bg-blue-50"}`}
                    >
                      {coupon.vendor_id ? "VENDOR SPECIFIC" : "PLATFORM WIDE"}
                    </span>
                  </div>
                ...
                <div className="mb-4">
                  ...
                  <p className="text-gray-400 text-xs line-clamp-2 min-h-[32px]">
                    {coupon.description || "Global store discount campaign."}
                  </p>
                </div>
                ...
                {/* Coupon Code Box */}
                <div className="bg-gray-50 border-2 border-dashed border-gray-200 rounded-2xl p-2 flex justify-between items-center mb-4">
                  <span className="font-mono font-bold text-xl text-main tracking-tighter">
                    {coupon.code}
                  </span>
                  <button
                    onClick={() => copyToClipboard(coupon.code)}
                    className="p-2 hover:bg-white rounded-lg transition-all text-gray-400 hover:text-main shadow-sm border border-transparent hover:border-gray-100"
                  >
                    <Copy size={18} />
                  </button>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4 mb-6 border-y border-gray-50 py-4">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <p className="text-sm font-bold text-gray-700">
                        ৳{parseFloat(coupon.minimum_amount || 0)}
                      </p>
                    </div>
                  </div>
                </div>
            ...
            <div className="p-4 max-h-[60vh] overflow-y-auto">
              <div className="space-y-3">
                {selectedCouponProducts.map((product, index) => (
                  <div
                    key={index}
                    className="flex items-center gap-4 p-2 bg-white border border-gray-100 rounded-2xl hover:border-main/30 transition-all shadow-sm group"
                  >
                    {/* Product Image */}
                    <div className="w-10 h-10 rounded-xl overflow-hidden bg-gray-100 border border-gray-50 shrink-0">
                      {product.image ? (
      ...
      {meta && meta.last_page > 1 && (
        <div className="mt-12 flex justify-center items-center gap-4">
          <button
            ...
            className="p-2 rounded-xl border bg-white disabled:opacity-30 shadow-sm transition-all hover:bg-gray-50"
          >
            <ChevronLeft size={20} />
          </button>
          <span className="text-sm font-bold text-gray-600 bg-white px-4 py-2 rounded-xl border shadow-sm">
