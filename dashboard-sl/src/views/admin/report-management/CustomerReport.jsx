      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-2 gap-4">
          ...
          <div className="flex gap-2">
            <button
              ...
              className="flex items-center gap-2 bg-white border px-4 py-1.5 rounded-lg hover:bg-gray-50 transition text-xs font-medium"
            >
              <Download size={16} /> CSV
            </button>
            <button
              onClick={() => downloadPDF(filteredCustomers, summary)}
              className="flex items-center gap-2 bg-main text-white px-4 py-1.5 rounded-lg hover:bg-mainHover transition text-xs font-medium"
            >
        ...
        {summary && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-2">
            ...
            <SummaryCard
              ...
              subValue={`LTV: ${summary.highest_ltv}`}
            />
            <SummaryCard
              icon={<TbCoinTaka className="text-green-600" />}
              label="Total Revenue"
              value={summary.total_lifetime_value}
            />
            <SummaryCard
              icon={<Calendar className="text-purple-600" />}
              label="Avg. Order Value"
              value={summary.average_ltv}
            />
          </div>
        )}
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
          ...
          <div className="p-4 bg-gray-50 border-t flex justify-between items-center text-xs text-gray-500">
            <span>
              Showing {filteredCustomers.length} of {meta.total} results
            </span>
            <div className="flex gap-2">
              <button
...
const SummaryCard = ({ icon, label, value, subValue }) => (
  <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100 flex items-center gap-4">
    ...
    <div>
      ...
      {subValue && (
        <p className="text-xs text-indigo-500 font-medium mt-0.5">{subValue}</p>
      )}
    </div>
  </div>
);
