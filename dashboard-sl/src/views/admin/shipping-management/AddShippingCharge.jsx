...
const AddShippingCharge = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      payload.is_countrywide = payload.is_countrywide ? 1 : 0;

      // 🔥 Convert numbers (or remove if empty)
      const numberFields = [
        "charge", // ✅ MUST ADD
        "min_weight",
        "max_weight",
      ...
      numberFields.forEach((field) => {
        if (payload[field] === "" || payload[field] === null) {
          delete payload[field]; // ❗ REMOVE করলে validation safe
        } else {
          payload[field] = Number(payload[field]); // convert to number
        }
      });

      // 🔥 Type based clean (VERY IMPORTANT)
      if (payload.type !== "weight") {
        delete payload.min_weight;
        delete payload.max_weight;
      }

      if (payload.type !== "quantity") {
        delete payload.min_quantity;
        delete payload.max_quantity;
      }
      ...
      // 🔥 Areas logic
      if (payload.is_countrywide === 1 || payload.is_countrywide === true) {
        payload.areas = null;
      } else {
        payload.areas = Array.isArray(payload.areas) ? payload.areas : [];
      }
      await API.post("/shipping-charges", payload);

      toast.success("Shipping charge created successfully!");
      navigate("/admin/shipping/charges");
    } catch (err) {
      // console.log(err.response?.data); // 🔥 MUST SEE THIS
  ...
  return (
    <div className="max-w-7xl mx-auto px-6 mb-4">
      ...
      <div className="flex items-center justify-between mb-4">
        <button
          onClick={() => navigate(-1)}
          className="flex items-center gap-2 text-gray-500 hover:text-gray-800 font-bold transition-all"
        >
          <ArrowLeft size={20} /> Back
        </button>
        <h1 className="text-xl  text-gray-900 uppercase">
      ...
      >
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6 bg-white p-4 md:p-8 rounded-2xl shadow-sm border border-gray-100">
          ...
          <div className="col-span-1 md:col-span-2 lg:col-span-1">
            <label className="block text-[10px] md:text-xs tracking-widest mb-2 uppercase font-semibold text-gray-500">
              Method Name
            </label>
            <input
              required
              ...
              {formData.type === "weight" && (
                <>
                  <input
                    ...
                    onChange={handleChange}
                    className="w-full px-5 py-2 border border-gray-200 rounded-xl outline-none text-sm focus:border-main"
                  />
                  <input
                    type="number"
                    step="0.01"
              ...
              <div className="col-span-1 sm:col-span-3 grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div className="space-y-1">
                  ...
                  <input
                    ...
                    onChange={handleChange}
                    className="w-full px-5 py-2 border border-gray-200 rounded-xl focus:ring-2 focus:ring-main outline-none text-sm font-bold text-main"
                  />
                </div>
                ...
                <div className="space-y-1">
                  ...
                  <input
                    ...
                    onChange={handleChange}
                    className="w-full px-5 py-2 border border-gray-200 rounded-xl focus:ring-2 focus:ring-main outline-none text-sm"
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-2">
              ...
              <label className="flex items-center gap-2 cursor-pointer bg-blue-50/50 px-3 py-1.5 rounded-lg border border-blue-100">
                <input
                  type="checkbox"
                  name="is_countrywide"
                  checked={formData.is_countrywide}
                  onChange={handleChange}
              ...
              <div className="space-y-4">
                ...
                <div className="flex flex-wrap gap-2">
                  {formData.areas.map((area, index) => (
                    <span
                      key={index}
                      className="flex items-center gap-2 bg-gray-50 border border-gray-200 px-3 py-1.5 rounded-lg text-xs font-bold text-gray-600 group"
                    >
                      {area}
                      <button
                        type="button"
            ...
            <div className="flex items-center gap-4 w-full sm:w-auto">
              ...
              <div className="flex items-center gap-2">
                ...
                <input
                  ...
                  onChange={handleChange}
                  className="w-16 px-2 py-1.5 bg-gray-50 border border-gray-200 rounded-lg outline-none font-bold text-center"
                />
              </div>
            </div>
          </div>
        ...
        <div className="flex justify-center md:justify-end">
          ...
          >
            {loading ? (
              ...
              <>
                <Save size={20} /> Save Shipping Method
              </>
            )}
          </button>
        </div>
