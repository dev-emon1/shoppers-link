...
import { Plus, Edit2, Trash2, ToggleLeft, ToggleRight, Image, Link2, X, Upload } from "lucide-react";
...
const SliderPage = () => {
    ...
    return (
        <div className="px-6 max-w-7xl mx-auto">
                ...
                <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 animate-fadeIn mt-10">
                    <div className="bg-white rounded-lg shadow-xl max-w-3xl w-[95%] p-5 relative overflow-auto max-h-[90vh]">
                        ...
                        <form onSubmit={handleSubmit} className="p-4 space-y-2">
                            {/* Title */}
                            <div>
                                <label className="block text-sm font-semibold text-gray-700 mb-700 mb-2">Title <span className="text-red-500">*</span></label>
                                <input
                                    required
                                    type="text"
            ...
            {
                loading ? (
                    <div className="text-center py-32 text-gray-500 text-xl">Loading slider...</div>
                ) : banners.length === 0 ? (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">
                        {banners.map((banner) => (
                            <div
                                key={banner.id}
                                className={`group relative bg-white rounded-2xl shadow-xl overflow-hidden border-4 transition-all ${banner.is_active ? "border-green-400" : "border-gray-300 opacity-80"
                                    }`}
                            >
                                <div className="relative h-48 overflow-hidden">
                                    ...
                                    {!banner.is_active && (
                                        <div className="absolute inset-0 bg-black bg-opacity-60 flex items-center justify-center">
                                            <span className="text-white text-3xl font-bold">INACTIVE</span>
                                        </div>
                                    )}
                                </div>

                                <div className="p-2">
                                    <h3 className="text-md font-bold text-gray-800 mb-2">{banner.title}</h3>
                                    {banner.description && <p className="text-gray-600 text-sm">{banner.description}</p>}
                                    ...
                                    <div className="flex justify-between items-center mt-4">
                                        ...
                                        <div className="flex gap-3">
                                            <button
                                                ...
                                                title="Toggle status"
                                            >
                                                {banner.is_active ? <ToggleRight size={18} /> : <ToggleLeft size={18} />}
                                            </button>
                                            <button
                                    ...
                                    <p className="text-sm text-gray-500 mt-4">
                                        Created: {format(new Date(banner.created_at), "dd MMM yyyy 'at' hh:mm a")}
                                    </p>
                                </div>
                            </div>
                        ))}
