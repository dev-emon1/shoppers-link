...
const VendordataPage = () => {
  ...
  const filteredVendors = vendors.filter((v) => {
    ...
    // ✅ ONLY ACTIVE VENDORS
    const isActive = v.user?.status === STATUS.ACTIVE;
  ...
  const handleSuspend = async (vendorId) => {
    ...
    try {
      ...
      updateVendorStatus(vendorId, STATUS.SUSPENDED);
  ...
  return (
    <div className="px-6 bg-gray-50 min-h-screen">
      ...
      <div className="flex flex-col md:flex-row justify-between items-center my-4 gap-4">
        <div>
          <h1 className="text-xl font-bold text-gray-800">Active Partners</h1>
          <p className="text-gray-500 text-sm">Manage active vendors</p>
        </div>
